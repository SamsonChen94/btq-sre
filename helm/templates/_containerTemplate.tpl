...
{{- $containerPortList = mustAppend $containerPortList $containerPort.containerPort }}
{{- end }}
{{- end }}
- {{ if .Values.argument -}}
  args:
  ...
  */ -}}
  ...
  {{- if .Values.lifecycle }}
  lifecycle:
    {{- $lifecycleArgs := dict "lifecycle" .Values.lifecycle "containerPortList" $containerPortList }}
    {{- include "lifecycle" $lifecycleArgs | indent 4 }}
  {{- end }}
  {{- if .Values.liveness }}
  livenessProbe:
