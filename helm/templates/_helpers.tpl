...
{{/*
  Note: Helm templates do not implement full-fledge functions. Some procedures
        defined in this template helper file contains an implementation of
        'returning' a result, abide in a very weird way
...
{{/*
  Create a fully qualified secret resource name for image pull secrets.
  Truncate at 63 characters because some Kubernetes name fileds are limited to
  this by the DNS naming specifications.
*/}}
{{- define "imagePullSecretName" -}}
{{- printf "%s-%s-secret" ( include "fullname" $ ) ( .Values | lower ) | trunc 63 | trimSuffix "-" -}}
{{- end -}}
...
{{/*
  Check if advanced scheduling is defined and enabled. Returns an array of
  boolean values depending on which scheduling configuration is enabled.
*/}}
...
{{/*
  Check if any probes are defined. Also does the following validation checks:
    1. Ensures that 'tcpSocket', 'httpGet', and 'exec' are the only accepted actions
    2. Ensures that 'tcpSocket', 'httpGet', and 'exec' are mutually exclusive
    3. Ensures that the HTTP port defined in 'httpGet' is also defined in 'services'
*/}}
{{- define "probe" }}
  {{- if .probe }}
    {{- if .probe.action }}
...
{{/*
  Check if lifecycle is defined. Also does the following validation checks:
    1. Ensures that 'postStart' and 'preStop' are the only accepted values
    2. Ensures that 'httpGet' and 'exec' are the only accepted nested values
    3. Ensures that 'httpGet' and 'exec' are mutually exclusive
    4. Ensures that the HTTP port defined in 'httpGet' is also defined in 'services'
*/}}
{{- define "lifecycle" }}
  {{- if .lifecycle.postStart }}
    {{- if .lifecycle.postStart.httpGet }}
      {{- if or ( empty .containerPortList ) ( not ( has .lifecycle.postStart.httpGet.port .containerPortList ) ) }}
        {{- fail ( printf "\n\nError --> %s declared as HTTP lifecycle postStart port but not defined in 'services'. Maybe use 'services.none'?" ( .lifecycle.postStart.httpGet.port | toString ) ) }}
      {{- end }}
    {{- end }}
  {{- end }}
  {{- if .lifecycle.preStop }}
    {{- if .lifecycle.preStop.httpGet }}
      {{- if or ( empty .containerPortList ) ( not ( has .lifecycle.preStop.httpGet.port .containerPortList ) ) }}
        {{- fail ( printf "\n\nError --> %s declared as HTTP lifecycle preStop port but not defined in 'services'. Maybe use 'services.none'? %s" ( .lifecycle.preStop.httpGet.port | toString ) ( .containerPortList | toString ) ) }}
      {{- end }}
    {{- end }}
  {{- end }}
  {{- range $key, $value := .lifecycle }}
    {{- if and ( ne $key "postStart" ) ( ne $key "preStop" ) }}
      {{- fail ( printf "\n\nError --> Invalid lifecycle event type detected. Got: %s" $key ) }}
    {{- end }}
    {{- if gt ( len $value ) 1 }}
      {{- fail ( printf "\n\nError --> Only one action allowed per lifecycle action. Got:\n%s" ( toYaml $value ) ) }}
    {{- end }}
    {{- if not ( or ( hasKey $value "exec" ) ( hasKey $value "httpGet" ) ) }}
      {{- fail ( printf "\n\nError --> Invalid lifecycle action. Got: %s" ( keys $value | first ) ) }}
    {{- end }}
{{ $key }}:
...
{{/*
  Check if image pull secrets are defined and enabled. Returns
  'pullSecretsEnabled' as a boolean and 'listOfKeys' as a list.
*/}}
{{- define "imagePullSecretsEnabledCheck" -}}
  {{- $check := false -}}
  {{- $listOfImagePullSecretKeys := (list) -}}
  {{- $listOfImagePullResourceNames := (list) -}}
  {{- if .pullSecrets.imagePullSecretKeys -}}
    {{- $check = true -}}
    {{- $listOfImagePullSecretKeys = mustAppend $listOfImagePullSecretKeys ( .pullSecrets.imagePullSecretKeys | first ) -}}
  {{- end -}}
  {{- range $additionalContainer := .pullSecrets.additionalContainers -}}
    {{- if $additionalContainer.imagePullSecretKeys }}
      {{- $check = true -}}
      {{- $listOfImagePullSecretKeys = mustAppend $listOfImagePullSecretKeys ( $additionalContainer.imagePullSecretKeys | first ) -}}
    {{- end -}}
  {{- end -}}
  {{- $_ := set .pullSecretsOutput "pullSecretsEnabled" $check }}
  {{- $_ := set .pullSecretsOutput "listOfKeys" $listOfImagePullSecretKeys }}
{{- end -}}
