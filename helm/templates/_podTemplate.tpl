...
{{- $imagePullSecretArgs := dict "pullSecrets" .Values "pullSecretsOutput" (dict) }}
{{- include "imagePullSecretsEnabledCheck" $imagePullSecretArgs }}
template:
  metadata:
    {{- /*
  ...
  spec:
    containers:
      ...
      */}}
      ...
      {{- $_ := set $additionalContainer "services" $.Values.services }}
    ...
    {{- if $imagePullSecretArgs.pullSecretsOutput.pullSecretsEnabled }}
    imagePullSecrets:
      {{- range $value := $imagePullSecretArgs.pullSecretsOutput.listOfKeys }}
      {{- $imagePullSecretNameArgs := dict "Release" $.Release "Values" $value }}
      - name: {{ include "imagePullSecretName" $imagePullSecretNameArgs }}
      {{- end }}
    {{- end }}
